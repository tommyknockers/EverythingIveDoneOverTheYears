...
(defn scrape-related-games-page [url]
  (let [data (get-some url)
        ...
        same-universe-url (sean-replace url "/related/" "/related/universe/")
        
                          ;(href (html/select data #{[:#main :div.relatedGamesNav :div.relatedGamesNavWrap :div.navItems :ol.navItems :li.universeNavItem :a]}))
...
(defn scrape-gamespot [url]
  (let [; get the page in tree form
        ...
        details-url (sean-replace url "/platform/" "/techinfo/platform/")
                    ;(href (html/select data 
        ...
        reviews-url (sean-replace url "/platform/" "/reviews/platform/")
        ...
        related-games-url (sean-replace url "/platform/" "/related/platform/")
...
;(fn [url2]
  ;(let [url  url2]
    ...
    ;(println "scraping momentarily paused due to gamespot website layout change")
    ;(. Thread (sleep 600000))
