...
#@login_required
def import_abner(request, project_id, user_id, doc_id, abner_input):
    #if not request.user.is_superuser:
        #return HttpResponseForbidden("Error: You must be an administrator to use this form")
    #if not request.FILES.has_key('fileToUpload') or not request.FILES['fileToUpload']:
        #return projectEdit(request,project_id=project_id,error="Please select a file to upload (supported formats are .txt).")
    #f = request.FILES['fileToUpload']
    #if (not f.name.endswith('.txt')):
        #return projectEdit(request,project_id=project_id,error="Please select a file to upload (supported formats are and .txt).")
    ...
    luser = Annotator.objects.get(id=user_id)

    # get an array of entities with words & tokens
    #abner_input = f.read()
    entities = abner_input.split(' ')
    new_entities = []
    for e in range(len(entities)):
        ...
        if entities[e].find('\n') != -1:
            continue
        word = entities[e].split('|')[0]
        try:
            token = entities[e].split('|')[1]
        except:
            pdb.set_trace()
        new_entities.append({'word':word, 'token':token})
    entities = new_entities
    del new_entities

    #pdb.set_trace()
    # for every entity, get indices from orig. doc
    begin = 0
    for e in range(len(entities)):
        try:
            begin = doc.text.find(entities[e]['word'], begin)
        except:
            pdb.set_trace()
        entities[e]['indices'] = [begin, begin+len(entities[e]['word'])]
        begin += len(entities[e]['word'])
    ...
    cur_ann = None
    for e in entities:
        if e['token'].startswith('B-'):
            cur_ann = Annotation()
            cur_ann.document = doc
            cur_ann.annotation_type = AnnotationType.objects.filter(project__id=project_id, type=e['token'][2:len(e['token'])])[0]
            cur_ann.annotator=luser
            cur_ann.begin_index = e['indices'][0]
            cur_ann.end_index = e['indices'][1]
            cur_ann.annotation = doc.text[cur_ann.begin_index:cur_ann.end_index]
        if e['token'].startswith('I-'):
            try:
                cur_ann.end_index = e['indices'][1]
            except:
                pdb.set_trace()
            cur_ann.annotation = doc.text[cur_ann.begin_index:cur_ann.end_index]
        if e['token'].startswith('O'):
...
#@login_required
def export_abner(request, project_id, user_id=None):
    docs = Document.objects.filter(project__id=project_id)
    #if doc_id:
        #docs = docs.filter(id=doc_id)
    response = []
    for doc in docs:
        print "splitting doc %s into sentences.." % (doc.title)
        # split doc.text into sentences
        nltk.data.path.insert(0,os.getcwd())
        tokenizer = nltk.data.load('english.pickle')
        sentences = tokenizer.tokenize(doc.text)
        for s in range(len(sentences)):
            sentences[s] = {'sentence': sentences[s]}
        print "done!"

        # get indices of every sentence in doc.text
        begin = 0
        print "getting indices of sentences.."
        for s in range(len(sentences)):
            begin = doc.text.find(sentences[s]['sentence'], begin)
            sentences[s]['indices'] = [begin, begin + len(sentences[s]['sentence'])]
            begin += len(sentences[s])
        print "done!"
        
        print "splitting doc into words.."
        # split doc.text into words
        words = nltk.word_tokenize(doc.text)
        #pdb.set_trace()
        for w in range(len(words)):
            words[w] = {'word':words[w]}
        print "done!"
        
        print "getting indices of words.."
        # get indices of every word in doc.text
        begin = 0
        for w in range(len(words)):
            begin = doc.text.find(words[w]['word'], begin)
            words[w]['indices'] = [begin, begin+len(words[w]['word'])]
            begin += len(words[w]['word'])
        print "done!" 
        
        print "tying words to sentences.."
        # tie words in each sentence to the sentence
        for w in words:
            for s in sentences:
                if w['indices'][0] >= s['indices'][0] and w['indices'][1] <= s['indices'][1]:
                    if 'words' not in s:
                        s['words'] = []
                    s['words'].append(w)
        print "done!"
        ...
        # get annotations
        annotator = Annotator.objects.get(id=user_id)
        annotations = Annotation.objects.filter(document__id=doc.id, annotator__id=annotator.id).order_by('begin_index')
        
        # assign an annotation to each word in the sentence if it has an annotation
        print "matching words and sentences to annotations.."
        for s in sentences:
            if 'words' not in s:
                s['words'] = [{'word':s['sentence']}]
                s['words'][0]['indices'] = [s['indices'][0], s['indices'][1]]
            for w in s['words']:
                for a in annotations:
                    if w['indices'][0] > a.end_index:
                        continue
                    if w['indices'][1] < a.begin_index:
                        break
                    if w['indices'][0] >= a.begin_index and w['indices'][1] <= a.end_index:
                        w['annotation']=a.annotation_type
        print "done!" 

        # print out each word in abner format
        # separate sentences by a newline
        # separate documents by two newlines
        print "spitting stuff out.."
        state = "read_initial_ann" # states are "read_initial_ann" and "read_next_ann"
        last_ann_type = None
        for s in sentences:
            for w in s['words']:
                if state == "read_initial_ann":
                    if 'annotation' in w:
                        response.append(w['word'] + "|B-" + w['annotation'].type)
                        last_ann_type = w['annotation']
                        state = "read_next_ann"
                        continue
                    else:
                        response.append(w['word'] + "|O")
                        continue
                if state == "read_next_ann":
                    if 'annotation' in w:
                        if w['annotation'] == last_ann_type:
                            response.append(w['word'] + "|I-" + w['annotation'].type)
                            continue
                        else:
                            response.append(w['word'] + "|B-" + w['annotation'].type)
                            last_ann_type = w['annotation']
