...
def _listSpan(annotator1, annotator2, doc, annotationType, infoType, f):
    annotations1,annotations2=Annotation.objects.filter(document=doc,annotator=annotator1, annotation_type=annotationType),Annotation.objects.filter(document=doc,
                                                                                                  annotator=annotator2, annotation_type=annotationType)
    inds1,inds2=map(_t,annotations1),map(_t,annotations2)
    result=[]
    if infoType==TP:
        #hits
        for x in range(len(inds1)):
            t=inds1[x]
            for i in inds2:
                if f(t,i):
                    result.append(annotations1[x])
    elif infoType==FP:
        #misses2
        for x in range(len(inds2)):
            t=inds2[x]
            skip=False
            for i in inds1:
                if f(t,i):
...
def listExactSpan(annotator1, annotator2, doc, annotatiionType, infoType):

    return _listSpan(annotator1, annotator2, doc, annotatiionType, infoType,lambda x,y: x==y)

def listOverlapSpan(annotator1, annotator2, doc, annotatiionType, infoType):

    return _listSpan(annotator1, annotator2, doc, annotatiionType, infoType,_overlap)
...
def listAnnotations(annotator1,annotator2, project, exact, infoType, annotationName):
    result=[]
    annotationType=AnnotationType.objects.filter(project=project,type=annotationName)[0]
    docs=annotator1.documents.filter(project=project)
    for doc in docs:
        if exact=="1":
            result.extend(listExactSpan(annotator1, annotator2, doc, annotationType, int(infoType)))
        else:
            result.extend(listOverlapSpan(annotator1, annotator2, doc, annotationType, int(infoType)))
    return result
