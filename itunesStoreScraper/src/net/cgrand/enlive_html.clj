...
(defn as-str [node-or-nodes]
  (apply str (emit* node-or-nodes)))
...
(defn select-text
  ...
  [node-or-nodes selector]
  (-> node-or-nodes (select selector) first text))

(defn select-attr
  "returns the attribute of the first matching node"
  [node-or-nodes selector attr]
  (-> node-or-nodes (select selector) first :attrs attr))
